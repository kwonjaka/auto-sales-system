...
export default function EditCustomerPage() {
  ...
  useEffect(() => {
    fetch(`/api/v1/customers/${id}`, { headers: authHeaders() })
      .then((r) => r.json()).then((j) => j.success && setForm({
        companyName: j.data.companyName, contactName: j.data.contactName,
        phone: j.data.phone ?? "", email: j.data.email ?? "",
        address: j.data.address ?? "", industry: j.data.industry ?? "",
      }));
  }, [id]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/customers/${id}`, { method: "PUT", headers: authHeaders(), body: JSON.stringify(form) });
  ...
  return (
    <AppLayout>
      <div className="page-header">
        <div>
          <h1 className="page-title">고객 수정</h1>
          <p className="page-sub">고객 정보를 수정합니다</p>
        </div>
      </div>
